...
def make_laptime_and_success_barplot():
    ...
    set_number = 4
    folder = base_path + test_name + f"_{set_number}/"
    ...
    for z in range(2):
        ...
        mins, maxes, means = load_time_data(folder, "positions")
        
        axs[z].bar(br1, means[key][0:4], color=pp_light[1], width=barWidth, label="Positions")
        plt.sca(axs[z])
        plot_error_bars(br1, mins[key][0:4], maxes[key], pp_darkest[1], w)
        
        mins, maxes, means = load_time_data(folder, "waypoints")
        axs[z].bar(br2, means[key][0:4], color=pp_light[5], width=barWidth, label="Waypoints")
    ...
    name = folder + f"{test_name}_Barplot"
