...
def make_TrainingGraph():
    ...
    id_names = ["state", "waypoints", "lidar", "lidarWaypoints", "stateWaypoints", "stateLidar", "stateLidarWaypoints"]
    print_names = ["S", "W", "L", "L+W", "S + W", "S + L", "S + L + W"]
    # print_names = ["x", "W", "L", "LW", "stateWaypoints", "stateLidar", "stateLidarWaypoints"]
    ...
    for i, id_name in enumerate(id_names): 
        ...
        for j in range(n_repeats):
            path = p + f"AgentOff_SAC_GameAblation_esp_{id_name}_5_{set_number}_{j}/"
    ...
    for i in range(len(steps_list)):
        ...
        plt.plot(xs, mean, '-', color=pp[i], linewidth=2, label=print_names[i])
