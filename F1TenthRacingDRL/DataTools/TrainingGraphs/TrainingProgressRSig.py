...
def make_training_progress_plot():
    ...
    fig, axs = plt.subplots(1, len(reward_list), figsize=(1 + 2*len(reward_list), 2.1))
    
    for m, reward_signal in enumerate(reward_list):
        steps_list = []
        progresses_list = []
        for a, vehicle_key in enumerate(vehicle_keys):
            ...
            for j in range(n_repeats):
                path = base_path + f"AgentOff_{alg}_{vehicle_key}_{map_name}_{reward_signal}_{max_speed}_{set_number}_{j}/"
