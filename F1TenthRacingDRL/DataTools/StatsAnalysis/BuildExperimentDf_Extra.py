...
def build_experiment_df(path):
    ...
    for j, agent_path in enumerate(vehicle_folders):
        ...
        try:
            df = pd.read_csv(agent_path + "ExtraAgentData.csv")
        ...
        for test_map in df.TestMap.unique():
            ...
            agent_data["LateralD_M"] = completed_data["LateralD_M"].mean()
            agent_data["LateralD_Q1"] = completed_data["LateralD_Q1"].mean()
            agent_data["LateralD_Q3"] = completed_data["LateralD_Q3"].mean()

            agent_data["SpeedD_M"] = completed_data["SpeedD_M"].mean()
            agent_data["SpeedD_Q1"] = completed_data["SpeedD_Q1"].mean()
            agent_data["SpeedD_Q3"] = completed_data["SpeedD_Q3"].mean()
    ...
    experiment_df.to_csv(path + "ExtraData.csv", index=False)
