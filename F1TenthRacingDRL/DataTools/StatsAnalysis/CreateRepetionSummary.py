...
class VehicleData:
    ...
    def save_data(self, map_name):
        ...
        with open(self.prefix + "RepetitionSummary_" + self.vehicle_id + f"_test{map_name[-3:].upper()}.txt", 'w') as file:
            file.write(f"Metric  , Time              , Success Rate     , Avg Progress    \n")
            for i in range(len(times)):
                file.write(f"{i}".ljust(10))
                file.write(f", {times[i]:14.4f}")
                file.write(f", {success_rates[i]:14.4f}")
                file.write(f", {progresses[i]:14.4f} \n")
...
def create_repetition_summary(path, n=3):
